...
public class cell_slime extends cell_zombie{
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Monster.createMonsterAttributes().add(Attributes.MAX_HEALTH, 40).add(Attributes.MOVEMENT_SPEED, 0.15).add(Attributes.KNOCKBACK_RESISTANCE, 0.2).add(Attributes.ATTACK_DAMAGE, 5);
    }
    public int getSize() {
    ...
    public void tick() {
        ...
        if (this.getHealth()<=1&& !(this.getMaxHealth() < 2)){
            ...
            if (this.getSize() > 0.25) {
                ...
                cell_slime.setYRot(this.getYRot());
                cell_slime.setXRot(this.getXRot());
                cell_slime.setTarget(this.getTarget());
