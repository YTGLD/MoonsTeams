...
public class SEmissiveLay<T extends small_zombie, M extends SModel<T>> extends RenderLayer<T, M> {
    private final ResourceLocation texture;
    private final AlphaFunction<T> alphaFunction;
    private final DrawSelector<T, M> drawSelector;

    public SEmissiveLay(RenderLayerParent<T, M> p_234885_, ResourceLocation p_234886_, AlphaFunction<T> p_234887_, DrawSelector<T, M> p_234888_) {
        super(p_234885_);
    ...
    @OnlyIn(Dist.CLIENT)
    public interface AlphaFunction<T extends small_zombie> {
        float apply(T p_234920_, float p_234921_, float p_234922_);
    }

    @OnlyIn(Dist.CLIENT)
    public interface DrawSelector<T extends small_zombie, M extends EntityModel<T>> {
