...
public class ambush extends TheNecoraIC {
    ...
    public void appendHoverText(ItemStack stack, @javax.annotation.Nullable Level level, List<Component> tooltip, TooltipFlag flags) {
        ...
        if (Screen.hasShiftDown()) {
            ...
            tooltip.add(Component.translatable("· 可以在任何静止的液体上行走").withStyle(ChatFormatting.RED));
        }else {
            tooltip.add(Component.translatable(""));
            tooltip.add(Component.translatable("-[SHIFT]").withStyle(ChatFormatting.DARK_RED));
            tooltip.add(Component.translatable("使僵尸的部位腐化导致其身躯膨胀能够在液体上漂流而行").withStyle(ChatFormatting.RED));
        }
    }
}
