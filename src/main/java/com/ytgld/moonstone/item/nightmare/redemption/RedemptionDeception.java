...
public class RedemptionDeception extends NightmareSmall {
    ...
    public static class ConfigItem implements RegisterItemConfig {
        ...
        public void config(ModConfigSpec.Builder builder) {
            intValue =  builder.translation("chest_item.config.RedemptionDeception")
    ...
    public static void LivingHurtEvent(LivingDamageEvent.Pre event) {
        if (event.getEntity() instanceof Player player) {
            if (SIHandler.hascurio(player, InitItems.nightmare_base_redemption_deception.get())) {
                if (!player.getCooldowns().isOnCooldown(InitItems.nightmare_base_redemption_deception.get().getDefaultInstance())) {
                    if (event.getNewDamage() > player.getHealth()) {

                        player.heal(player.getMaxHealth() * (ConfigItem.intValue.get().floatValue() / 100f));


                        player.getCooldowns().addCooldown(InitItems.nightmare_base_redemption_deception.get().getDefaultInstance(), 1200);
