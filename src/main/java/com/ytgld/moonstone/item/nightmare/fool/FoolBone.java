package com.ytgld.moonstone.item.nightmare.fool;
    ...
    public static void attLook(LivingDamageEvent.Pre event) {
        if (event.getSource().getEntity() instanceof Player player) {
            if (SIHandler.hascurio(player, InitItems.nightmare_base_fool_bone.get())) {
                if (!player.getCooldowns().isOnCooldown(InitItems.nightmare_base_fool_bone.get().getDefaultInstance())) {
                    if (event.getEntity() instanceof Mob mob) {
                        if (mob.getTarget() != null && mob.getTarget().is(player)) {
                            ...
                            player.getCooldowns().addCooldown(InitItems.nightmare_base_fool_bone.get().getDefaultInstance(),20);
                        }
                    }
                }
            }
