    ...
    public boolean overrideOtherStackedOnMe(ItemStack me, ItemStack Other, Slot p_150744_, ClickAction p_150745_, Player p_150746_, SlotAccess p_150747_) {
        if (p_150745_ == ClickAction.SECONDARY && p_150744_.allowModification(p_150746_)) {
            if (Other.isEmpty()) {
                ...
                boolean canFlySword = tag.getBoolean(RineSword.canFlySword); // 假设"canFlySword"是一个字符串常量
                        ...
                        for (int i = 0; i < stacksHandler.getSlots(); i++) {
                            ...
                            if (stack.is(doomswoud.get())) {
                                if (stack.get(DataReg.tag) != null) {
                                    if (!stack.get(DataReg.tag).getBoolean(SevenSword.canFlySword)) {
                                        if (!player.getCooldowns().isOnCooldown(doomswoud.get())) {
                                            for (int p = 0; p < 2; p++) {
                                                ...
                                                player.getCooldowns().addCooldown(doomswoud.get(), 40);
                                            }
                                        }
                                    }
                                }
                                    ...
                                    if (stack.get(DataReg.tag) != null) {
                                        if (!stack.get(DataReg.tag).getBoolean(SevenSword.canFlySword)) {
                                            ...
                                            for (int p = 0; p < 4; p++) {
                                                ...
                                                item.setTarget(event.getEntity());
                                                player.level().addFreshEntity(item);
                                                player.getCooldowns().addCooldown(doomswoud.get(), 50);
                                            }
                                        }
                                    }
                                }
