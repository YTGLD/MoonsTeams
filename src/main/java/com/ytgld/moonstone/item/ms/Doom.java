...
public class Doom extends CommonItem {
    ...
    public static void renderBack(RenderTooltipEvent.Color event) {
        if (event.getItemStack().getItem() instanceof Doom item) {
            event.setBackground(Light.ARGB.color(255, 5, 5, 25));
            event.setBorderEnd(Light.ARGB.color(255, 50, 50, 255));
            event.setBorderStart(Light.ARGB.color(255, 50, 50, 255));
        }
    }
}
