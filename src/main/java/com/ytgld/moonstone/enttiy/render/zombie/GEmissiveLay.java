...
public class GEmissiveLay <T extends CellGiant, M extends GModel<T>> extends RenderLayer<T, M> {
    ...
    @OnlyIn(Dist.CLIENT)
    public interface AlphaFunction<T extends CellGiant> {
        float apply(T p_234920_, float p_234921_, float p_234922_);
    }

    @OnlyIn(Dist.CLIENT)
    public interface DrawSelector<T extends CellGiant, M extends EntityModel<T>> {
