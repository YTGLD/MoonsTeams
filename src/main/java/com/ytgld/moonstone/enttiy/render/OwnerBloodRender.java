...
public class OwnerBloodRender extends EntityRenderer<@NotNull OwnerBlood> {
    public OwnerBloodRender(EntityRendererProvider.Context p_173917_) {
        super(p_173917_);
    }

    @Override
    public boolean shouldRender(OwnerBlood livingEntity, Frustum camera, double camX, double camY, double camZ) {
        return true;
    }

    @Override
    public ResourceLocation getTextureLocation(@NotNull OwnerBlood attackBlood) {
        return ResourceLocation.fromNamespaceAndPath(Moonstone.MODID, "");
    }

    @Override
    public void render(@NotNull OwnerBlood p_entity, float entityYaw, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        HandlerClient.showOutline = true;
        HandlerClient.doPass = true;

        OwnerBlood entity = p_entity;
    ...
    private void setT(PoseStack matrices, OwnerBlood entity, VertexConsumer vertexConsumers) {
    ...
    public static void renderBloodW(PoseStack poseStack, VertexConsumer vertexConsumer, Vec3 start, Vec3 end, int a, float r) {
    ...
    private static void addSquare(VertexConsumer vertexConsumer, PoseStack poseStack, Vec3 up1, Vec3 up2, Vec3 down1, Vec3 down2, int c) {
        // 添加四个顶点来绘制一个矩形
        vertexConsumer.addVertex(poseStack.last(), (float) up1.x, (float) up1.y, (float) up1.z)
                .setColor(c)
                .setUv2(255, 255)
                .setNormal(poseStack.last(), 0, 0, 1);

        vertexConsumer.addVertex(poseStack.last(), (float) down1.x, (float) down1.y, (float) down1.z)
                .setColor(c)
                .setUv2(255, 255)
                .setNormal(poseStack.last(), 0, 0, 1);

        vertexConsumer.addVertex(poseStack.last(), (float) down2.x, (float) down2.y, (float) down2.z)
                .setColor(c)
                .setUv2(255, 255)
                .setNormal(poseStack.last(), 0, 0, 1);

        vertexConsumer.addVertex(poseStack.last(), (float) up2.x, (float) up2.y, (float) up2.z)
                .setColor(c)
                .setUv2(255, 255)
                .setNormal(poseStack.last(), 0, 0, 1);
    }

    public void renderSphere1(@NotNull PoseStack poseStack, @NotNull VertexConsumer vertexConsumer, int light, float s) {
