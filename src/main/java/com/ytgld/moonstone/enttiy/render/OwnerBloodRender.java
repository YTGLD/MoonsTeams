...
import com.ytgld.moonstone.enttiy.state.OwnerBloodState;
...
public class OwnerBloodRender extends EntityRenderer<@NotNull OwnerBlood, OwnerBloodState> {
    ...
    @Override
    public @NotNull OwnerBloodState createRenderState() {
        return new OwnerBloodState();
    }

    @Override
    public void submit(OwnerBloodState renderState, PoseStack modelView , SubmitNodeCollector collector, CameraRenderState camera) {
        OwnerBlood entity = renderState.entity;
        submitOther(renderState, modelView, collector, camera);
    }
    public void submitOther(OwnerBloodState renderState, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState camera) {
        ...
        collector.submitCustomGeometry(poseStack, MRender.renderTypeOutline, (pose, bufferSource) -> {
            renderSphere1(pose, bufferSource, 0, 0.33f);
        });
    ...
    private void setT(PoseStack.Pose matrices, OwnerBlood entity, VertexConsumer vertexConsumers) {
    ...
    @Override
    public void extractRenderState(OwnerBlood entity, OwnerBloodState reusedState, float partialTick) {
